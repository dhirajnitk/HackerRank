...
template <typename K, typename V> class HashMap{
   //vector<HashNode<K,V>*> *bucketArray;
    std::unique_ptr<vector<HashNode<K,V>*>>bucketArray;
   int numBuckets;
   int size;
public:
    HashMap(){
        //bucketArray = new vector<HashNode<K,V>*>();
        bucketArray = make_unique<vector<HashNode<K,V>*>>();
        numBuckets = 10;
        size = 0;
        for( int i = 0;  i<numBuckets; i ++) 
            bucketArray.get()->emplace_back(nullptr);
    }
    ...
    V remove(K key){
        ...
        HashNode<K,V>* head = bucketArray.get()->at(bucketIndex);
        ...
        if(prev != nullptr)
            ...
            bucketArray.get()->at(bucketIndex) =  head->next;
        return head->value;
    }
    V get(K key){
        int bucketIndex = getBucketIndex(key);
    ...
    void add(K key, V value){
        ...
        size++;
        head = bucketArray.get()->at(bucketIndex);
        HashNode<K,V>* newNode = new HashNode<K,V>(key, value);
        newNode->next = head;
        bucketArray.get()->at(bucketIndex) =  newNode;
        if((1.0*size)/numBuckets >= 0.7){
            //vector<HashNode<K,V>*>*temp = bucketArray;
            std::unique_ptr<vector<HashNode<K,V>*>>temp = std::move(bucketArray);
            //bucketArray = new vector<HashNode<K,V>*>;
            bucketArray = make_unique<vector<HashNode<K,V>*>>();
            ...
            for(HashNode<K,V>* headNode : *(temp.get())){
